    ...
    public class MessageHandler : IUpdateHandler
    {
        ...
        public async Task HandleUpdate(Update update)
        {
            ...
            if (message.Text!.IsCommand(out Domain.Entities.Enums.BotCommand command))
            {
                ...
                switch (command)
                {
                    case Domain.Entities.Enums.BotCommand.AdminPanel:
                        {
                            if (!user.IsAdmin())
                                return;
                            await botClient.SendMessage(chatId, responseMessage.Message, replyMarkup: BotKeyboards.MainAdminKeyboard());
                        }
                        break;
                    default:
                        {
                            await botClient.SendMessage(chatId, responseMessage.Message, replyMarkup: BotKeyboards.MainKeyboard(user.IsAdmin()));
                        }
                        break;
                }
            }
